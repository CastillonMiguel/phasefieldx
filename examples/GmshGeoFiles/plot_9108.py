...
.geo File: Double Edge Notch Tension Test
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
This example demonstrates the procedure for generating a mesh for a double edge notched tension test.
...
.. include::  ../../../../examples/GmshGeoFiles/9108_DoubleEdgeNotchTensionTest/file.geo
   :literal:
...
Here, `-2` specifies that the mesh is 2-dimensional, and `-o mesh.msh` tells Gmsh to save the mesh to a file named `mesh.msh` with the `.msh` extension.
...
folder = "9108_DoubleEdgeNotchTensionTest"
