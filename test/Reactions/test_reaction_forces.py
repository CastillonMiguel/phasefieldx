...
def run_1d_simulation():
    Data = Input(E=210.0,
                ...
                results_folder_name="1_dimension")
    divx = 1
    lx  = 1
    msh = dolfinx.mesh.create_unit_interval(mpi4py.MPI.COMM_WORLD,divx)
    ...
    bc_bottom = bc_x(bottom_facet_marker, V_u, fdim, value=0.0)
    bc_top = bc_x(top_facet_marker, V_u, fdim, value=0.0)
...
def run_simulations():
    ...
    S_1d = AllResults("1_dimension")
    S_2d = AllResults("2_dimension")
    S_3d = AllResults("3_dimension")

    return S_1d, S_2d, S_3d

def test_reaction_forces(run_simulations):

    S_1d, S_2d, S_3d = run_simulations

    # Extract reaction forces
    reaction_1d = S_1d.reaction_files['bottom.reaction']["Rx"]
    ...
    np.testing.assert_allclose(reaction_1d, reaction_2d, rtol=1e-5, atol=1e-8, err_msg="1D and 2D reaction forces do not match")
    np.testing.assert_allclose(reaction_1d, reaction_3d, rtol=1e-5, atol=1e-8, err_msg="1D and 3D reaction forces do not match")
